...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      ...
      case (b, ms) #:: tail => {
        // This is commented out as it generates a stack overflow error
        //val newExplored = explored + b
        //val newNeighbors = newNeighborsOnly(neighborsWithHistory(b, ms), newExplored)
        //newNeighbors ++ from(tail ++ newNeighbors, newExplored)

        val newExplored = explored + b
        val newNeighbours = newNeighborsOnly(neighborsWithHistory(b, ms), newExplored)
        (b, ms) #:: from(tail ++ newNeighbours, newExplored)
      }
    }
  }
